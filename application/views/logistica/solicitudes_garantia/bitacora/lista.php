<style>
    #tabla_solicitudes_garantia_bitacora tbody td {
        font-size: 0.9em;
        padding: 5px;
    }
</style>
...
<table class="table-striped table-bordered" id="tabla_solicitudes_garantia_bitacora"></table>

<script>
    $().ready(async () => {
        let = tablaSolicitudesGarantia = $("#tabla_solicitudes_garantia_bitacora").DataTable({
            ajax: {
                url: `${$("#site_url").val()}logistica/obtener_datos_tabla`,
                data: datos => {
                    datos.tipo = 'solicitudes_garantia_bitacora'
                    datos.solicitud_id = $('#id_solicitud_garantia').val()
                },
            },
            columns: [
                {
